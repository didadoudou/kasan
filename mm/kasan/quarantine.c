...
#define QUARANTINE_FRACTION 32 /* Quarantine doesn't support memory shrinker
				  with SLAB allocator, therefore we set the
				  ratio lower to avoid OOM. */
#endif
...
static unsigned long quarantine_shrinker_count(struct shrinker *shrinker,
	...
	if (sc->gfp_mask & GFP_DMA)
		...
		return (ACCESS_ONCE(global_quarantine.bytes) >> PAGE_SHIFT)
			/ 16;
}


static unsigned long shrink_quarantine(struct qlist* q,
				       struct shrink_control *sc) {
	...
	while (prev && *prev && sc->nr_to_scan) {
		...
		if ((sc->nid == NUMA_NO_NODE || sc->nid == page_to_nid(page)) &&
			...
			qlink = qlist_remove(q, prev, cache->size);
...
static unsigned long quarantine_shrinker_scan(struct shrinker * shrinker,
	...
	if (sc->gfp_mask & __GFP_WAIT) {
		spin_lock_irqsave(&quarantine_lock, flags);
		freed = shrink_quarantine(&global_quarantine, sc);
		spin_unlock_irqrestore(&quarantine_lock, flags);
	} else {
		local_irq_save(flags);
		freed = shrink_quarantine(this_cpu_ptr(&cpu_quarantine), sc);
		local_irq_restore(flags);
	}
